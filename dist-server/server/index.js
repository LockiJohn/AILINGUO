...
const database_1 = require("../electron/db/database");
...
// Routes
// --- CONTENT CONTROLLERS ---
const contentLoader_1 = require("../electron/content/contentLoader");
app.post('/api/content/reload', (req, res) => {
    ...
    (0, contentLoader_1.loadUnitsForLevel)(db, levelCode);
    res.json({ ok: true });
});
// --- LESSON CONTROLLERS ---
...
app.get('/api/users/stats', (req, res) => {
    const stats = db.prepare('SELECT * FROM user_stats WHERE user_id = ?').get(currentUserId);
    ...
    const lessonsCompleted = db.prepare('SELECT COUNT(*) as count FROM lesson_progress WHERE user_id = ?').get(currentUserId).count;
...
app.post('/api/progress/exercise', (req, res) => {
    ...
    db.prepare('INSERT INTO exercise_results (exercise_id,user_id,user_answer,is_correct,response_time_ms) VALUES (?,?,?,?,?)').run(exerciseId, currentUserId, userAnswer, isCorrect ? 1 : 0, responseTimeMs);
    db.prepare(`
...
app.post('/api/progress/lesson', (req, res) => {
    ...
    if (stats) {
        ...
        db.prepare('UPDATE user_stats SET total_xp=?,current_streak=?,longest_streak=?,last_study_date=? WHERE user_id=?').run(newXp, newStreak, newLongest, today, currentUserId);
...
app.post('/api/progress/review-item', (req, res) => {
    ...
    const item = db.prepare('SELECT * FROM review_queue WHERE exercise_id=? AND user_id=?').get(exerciseId, currentUserId);
    if (!item)
...
app.post('/api/progress/session/end', (req, res) => {
    ...
    db.prepare(`UPDATE study_sessions SET ended_at=datetime('now'),xp_earned=?,exercise_count=?,accuracy=? WHERE id=? AND user_id=?`).run(xpEarned, exerciseCount, accuracy, sessionId, currentUserId);
    res.json({ ok: true });
});
...
function checkAndAwardBadges(db, totalXp, streak, accuracy, userId) {
    const award = (code) => {
    ...
    const lc = db.prepare('SELECT COUNT(*) as count FROM lesson_progress WHERE user_id=?').get(userId).count;
    if (lc >= 1)
        award('first_lesson');
    if (streak >= 3)
        award('streak_3');
